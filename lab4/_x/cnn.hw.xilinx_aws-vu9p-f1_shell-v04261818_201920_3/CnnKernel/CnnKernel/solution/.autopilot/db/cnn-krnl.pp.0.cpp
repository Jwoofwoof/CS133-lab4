# 1 "/home/centos/CS133-lab4/lab4/cnn-krnl.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 375 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/home/centos/CS133-lab4/lab4/cnn-krnl.cpp" 2
...
# 1 "/home/centos/CS133-lab4/lab4/lib/cnn-krnl.h" 1
...
# 55 "/opt/Xilinx/Vitis_HLS/2021.1/common/technology/autopilot/ap_fixed.h"
...
# 56 "/opt/Xilinx/Vitis_HLS/2021.1/common/technology/autopilot/ap_fixed.h" 2
...
# 56 "/opt/Xilinx/Vitis_HLS/2021.1/common/technology/autopilot/ap_int.h"
...
# 15 "/home/centos/CS133-lab4/lab4/lib/cnn-krnl.h" 2
# 1 "/opt/Xilinx/Vitis_HLS/2021.1/common/technology/autopilot/ap_int.h" 1
# 16 "/home/centos/CS133-lab4/lab4/lib/cnn-krnl.h" 2
# 31 "/home/centos/CS133-lab4/lab4/lib/cnn-krnl.h"
template <class T>
inline T max(T a, T b) { return a > b ? a : b; }
# 102 "/home/centos/CS133-lab4/lab4/lib/cnn-krnl.h"
...
void read_weight_from_memory(
    ...
    weight_t weight[(256)][(256)][(5)][(5)]) {

  const int total_read = (256) * (256) * (5) * (5) / item_per_read;

  read_weight:
  for (int idx = 0; idx < total_read; idx++) {
    weight_g_t data = weight_g[idx];

    VITIS_LOOP_125_1: for (int item = 0; item < item_per_read; item++) {
#pragma HLS pipeline
 int real_index = idx * item_per_read + item;
      int l = real_index % (5); real_index /= (5);
      int k = real_index % (5); real_index /= (5);
      int j = real_index % (256); real_index /= (256);
      int i = real_index;
...
void read_bias_from_memory(
  ...
  const int total_read = (256) / item_per_read;

  read_bias:
  for (int idx = 0; idx < total_read; idx++) {
    bias_g_t data = bias_g[idx];

    VITIS_LOOP_149_1: for (int item = 0; item < item_per_read; item++) {
#pragma HLS pipeline
 int real_index = idx * item_per_read + item;
      ap_uint<8> temp = (data >> (item * 8))(7, 0);
...
void read_input_from_memory(int hh, int ww,
    ...
    input_t input[(256)][(112)+4][(112)+4]) {

  read_input:
  for (int j = 0; j < (256); j++)
    VITIS_LOOP_164_1: for (int h = 0; h < (112) + 4; h++) {

      const int start_real = j*(228)*(228)+(hh+h)*(228) + ww;
      const int end_real = j*(228)*(228)+(hh+h)*(228) + ww+(112)+4;
      const int start = start_real / item_per_read;
      const int till = (end_real - 1) / item_per_read;

      VITIS_LOOP_171_2: for (int idx = start; idx <= till; idx++) {
...
#pragma HLS loop_tripcount min=2 max=2 avg=2
 ...
 input_g_t data = input_g[idx];
        ...
        VITIS_LOOP_183_3: for (int item = 0; item < item_per_read; item++) {
#pragma HLS pipeline
 int real_index = idx * item_per_read + item;
          if (real_index < end_real && real_index >= start_real) {
            int w = real_index % (228);
            ap_uint<8> temp = (data >> (item * 8))(7, 0);
...
void write_output_to_memory(int hh, int ww,
    output_g_t *output_g,
    output_t output[(256)][(112)/2][(112)/2]) {

  hh /= 2; ww /= 2;

  write_output:
  for (int i = 0; i < (256); i++)
    VITIS_LOOP_204_1: for (int h = 0; h < (112)/2; h++) {

      const int start_real = i*(112)*(112)+(hh+h)*(112) + ww;
      const int end_real = i*(112)*(112)+(hh+h)*(112) + ww+(112)/2;
      const int start = start_real / item_per_read;
      const int till = (end_real - 1) / item_per_read;

      VITIS_LOOP_211_2: for (int idx = start; idx <= till; idx++) {
...
#pragma HLS loop_tripcount min=1 max=1 avg=1
 ...
 output_g_t data = output_g[idx];
        ...
        VITIS_LOOP_220_3: for (int item = 0; item < item_per_read; item++) {
#pragma HLS pipeline
 int real_index = idx * item_per_read + item;
          if (real_index < end_real && real_index >= start_real) {
            int w = real_index % (112);
...
extern "C" __attribute__((sdx_kernel("CnnKernel", 0))) void CnnKernel(
    const input_g_t *input, const weight_g_t *weight,
    const bias_g_t *bias, output_g_t *output) {
#pragma HLS TOP name=CnnKernel
# 247 "/home/centos/CS133-lab4/lab4/lib/cnn-krnl.h"
...
#pragma HLS array_partition variable=output dim=3 cyclic
 ...
 static compute_t C[(112)][(112)];
  ...
  main_loop_tile_h:
  for (int hh = 0; hh < (224); hh += (112)) {

    main_loop_tile_w:
    for (int ww = 0; ww < (224); ww += (112)) {
      ...
      for (int i = 0; i < (256); ++i) {
        ...
        set_bias:
        for (int h = 0; h < (112); ++h) {
          VITIS_LOOP_52_1: for (int w = 0; w < (112); ++w)
            C[h][w] = bias[i];
        }
        ...
        conv:
        for (int j = 0; j < (256); ++j) {
          VITIS_LOOP_59_2: for (int h = 0; h < (112); ++h) {
            VITIS_LOOP_60_3: for (int w = 0; w < (112); ++w) {
#pragma HLS pipeline II=1
 VITIS_LOOP_62_4: for (int p = 0; p < (5); ++p) {
#pragma HLS unroll
 VITIS_LOOP_64_5: for (int q = 0; q < (5); ++q)
#pragma HLS unroll
 C[h][w] += weight[i][j][p][q] *
        ...
        relu:
        for (int h = 0; h < (112); ++h) {
          VITIS_LOOP_76_6: for (int w = 0; w < (112); ++w) {
            if (C[h][w] < 0) C[h][w] = 0;
          }
        }
        ...
        maxpool:
        for (int h = 0; h < (112)/2; ++h) {
          VITIS_LOOP_84_7: for (int w = 0; w < (112)/2; ++w) {
            output[i][h][w] = max(
